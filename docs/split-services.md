...
- Processes: `entry-worker-erc20.js` + `entry-worker-trc20.js` (deposit scanners; or combined `entry-worker.js`), `entry-cron-1.js` (maintenance: wallet-pool holds, abandoned checkout expiry, **auto-payout drain** every minute), `entry-cron-2.js` (tron sweep), deposit full-scan cron if enabled, or matching `npm run start:* -w cron` scripts.
- **Auto payout:** `POST /api/v1/gateway/payout` tries an on-chain send in-request; leftover `pending` / stuck `processing` rows are drained by maintenance cron (`cron/src/jobs/auto-payout-cron.js`). Hot-wallet keys: `PAYOUT_HOT_PRIVATE_KEY_TRON` / `PAYOUT_HOT_PRIVATE_KEY_ETH` (`.env` or Admin → **Payout · auto**).
