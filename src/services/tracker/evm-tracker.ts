...
function chainConfigKey(chain: Chain): string {
  return chain;
}

/**
 * EVM indexer: one JsonRpcProvider per chain, block polling with bounded batch size.
 * - Native: match tx.to + value > 0
 * - ERC20: single getLogs per block for Transfer events, filter tracked contracts + our addresses
 */
export async function scanEvmChain(chain: Chain): Promise<void> {
  ...
  const erc20Map: Record<string, { symbol: string; decimals: number }> = {};
