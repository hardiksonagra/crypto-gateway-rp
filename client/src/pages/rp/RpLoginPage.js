        ...
        >
          ...
          <Formik
            ...
            onSubmit={async (values, { setStatus, setSubmitting }) => {
              setStatus(undefined);
              try {
                const r = await api("/api/v1/auth/login/rp", {
                  method: "POST",
            ...
            {({ isSubmitting, status }) => (
              <Form className="space-y-5">
                <div>
                  ...
                  <Field id="rp-email" name="email" type="email" className="form-input" autoComplete="username" placeholder="you@example.com" />
                  <ErrorMessage name="email" component="p" className="mt-1 text-xs text-rose-400" />
                </div>
                <div>
                  <label className="form-label" htmlFor="rp-password">Password</label>
                  <Field id="rp-password" name="password" type="password" className="form-input" autoComplete="current-password" placeholder="••••••••" />
                ...
                <p className="text-center text-xs" style={{ color: "var(--text-3)" }}>
                  <Link to="/control/login" className="transition"
                        onMouseEnter={(e) => { e.currentTarget.style.color = "var(--link-active-color)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.color = "var(--text-3)"; }}>
